...
<head>
    ...
    <style>
        ...
        body::before {
            ...
            color: rgba(200, 200, 200, 0.3); /* Gris claro y semi-transparente */
            font-size: 120px;
            font-weight: bold;
            text-align: center;
            line-height: 1;
        ...
        /* Asegurar que la marca de agua no se repita en cada página al imprimir */
        @media print {
            body::before {
                content: "COTIZACIÓN";
                position: fixed;
...
<div class="container">
    ...
    <div class="datos-cliente">
        ...
        <div><strong>N° Cuotas:</strong> {{ $cotizacion->numero_cuotas }}</div>
        <div><strong>TEA:</strong> {{ number_format($cotizacion->tasa_interes * 100, 2) }}%</div>
        <div><strong>TEM:</strong> {{ number_format(($cotizacion->tasa_interes > 0 ? pow(1 + $cotizacion->tasa_interes, 1/12) - 1 : 0) * 100, 2) }}%</div>
    </div>
...
<div style="margin-top: 15px;">
    ...
    <table style="width: 100%; border-collapse: collapse; font-size: 10.5px; margin-top: 10px; border: 1px solid #000;">
        ...
        <tbody>
            <tr>
                <td style="text-align: right; padding: 3px 5px; border: 1px solid #000;">S/ {{ number_format($cotizacion->inicial, 2) }}</td>
                <td style="text-align: right; padding: 3px 5px; border: 1px solid #000;">S/ {{ number_format($cotizacion->monto_financiar, 2) }}</td>
    ...
    <div style="margin-top: 5px; font-size: 9px; font-style: italic;">
        {{ \Carbon\Carbon::parse($cotizacion->fecha_pago)->format('d/m/Y') }}: Cotización válida por 30 días.
    </div>
</div>
    <!-- Pie de página -->
    <div class="footer">
