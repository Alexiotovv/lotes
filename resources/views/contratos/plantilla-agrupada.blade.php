...
<head>
    ...
    <title>Contrato de Compra Venta - Múltiples Lotes</title>
    <style>
        button {
            margin-top: 15px;
            padding: 6px 12px;
...
<body>
    <div class="a4-container">
        ...
        <h2 style="text-align: center;">CONTRATO DE COMPRA VENTA DE LOTES CON DERECHOS POSESORIOS</h2>

        <p>MEDIANTE EL PRESENTE ACTO SE CELEBRA EL CONTRATO DE COMPROMISO DE COMPRA VENTA DE DERECHOS POSESORIOS DE INMUEBLES.</p>
        ...
        <ol>
            ...
            <table>
                <thead>
                    <tr>
                        <th>Lote</th>
                        <th>Frente (m)</th>
                        <th>Costado Derecho (m)</th>
                        <th>Costado Izquierdo (m)</th>
                        <th>Fondo (m)</th>
                        <th>Área (m²)</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    @php
                        $mz = substr($venta->lote->codigo, 0, 1);
                        $lt = substr($venta->lote->codigo, 1);
                    @endphp
                    <tr>
                        <td>{{ $venta->lote->codigo }} (Mz. {{ $mz }} Lote {{ $lt }})</td>
                        <td>{{ $venta->lote->frente }}</td>
                        <td>{{ $venta->lote->lado_derecho }}</td>
                        <td>{{ $venta->lote->lado_izquierdo }}</td>
                        <td>{{ $venta->lote->fondo }}</td>
                        <td>{{ $venta->lote->area_m2 }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
            ...
            <li>EL VENDEDOR ESTA OBLIGADO A VENDER AL COMPRADOR Y ESTE ESTARA OBLIGADO A COMPRAR 
                ...
                <ul>
                    ...
                    @php
                        ...
                        $lt = substr($venta->lote->codigo, 1);
                    @endphp
                    <li>LOTE N° {{ $lt }} UBICADO EN LA MZ. "{{ $mz }}"</li>
                    @endforeach
                </ul>
                EN UN PLAZO DE {{ $ventas->first()->numero_cuotas }} MESES,
                CONTADOS A PARTIR DEL DIA SIGUIENTE A LA FIRMA DE ESTE CONTRATO.</li>
                
            <li>LAS PARTES FIJAN POR MUTUO ACUERDO COMO PRECIO TOTAL DE LA FUTURA COMPRA VENTA LA
                CANTIDAD DE S/ {{ number_format($totalPrecio, 2) }} 
                ({{ \App\Helpers\NumerosALetras::convertir($totalPrecio) }} SOLES), 
                QUE TENDRA QUE SER CANCELADO EN {{ $ventas->first()->numero_cuotas }} 
                CUOTAS MENSUALES DE S/ {{ number_format($ventas->sum('cuota'), 2) }} 
                ({{ \App\Helpers\NumerosALetras::convertir($ventas->sum('cuota')) }}), 
                QUE ESTARA DETALLADA EN EL ANEXO DEL CONTRATO.</li>
            ...
            <li>EL VENDEDOR SE COMPROMETE A PROCEDER CON LA INDEPENDIZACION Y EL CAMBIO DE 
                ...
                TOTALIDAD DEL VALOR DE VENTA DE LOS RESPECTIVOS LOTES.</li>
                
            <li>EL VENDEDOR ASUME LOS GASTOS NOTARIALES.</li>
            
            <li>EL COMPRADOR ASUME LOS GASTOS DE INDEPENDIZACION DE SUS LOTES LO CUAL SE LE
            ...
            <li>EL COMPRADOR AL RECEPCIONAR LA ENTREGA DE LOS LOTES SE HACE RESPONSABLE DE LA 
                CONSERVACION Y CUIDADO DE SU PERIMETRO, ASI MISMO SE HACE RESPONSABLE DE 
                RECORDAR LA UBICACIÓN DE SUS LOTES Y ASUMIR LOS IMPUESTOS PREDIALES MUNICIPALES, 
                PESE A QUE ESTOS AÚN FIGUREN A NOMBRE DEL VENDEDOR.</li>
        ...
        <ol>
            <li>EL PRESENTE CONTRATO SE RESOLVERA A FAVOR DE LA VENDEDORA SI EL COMPRADOR INCUMPLE 
                CON EL PAGO DE 02 (DOS) CUOTAS CONSECUTIVAS DE LAS {{ $ventas->first()->numero_cuotas }} 
                (<span id="numero-cuotas-letras">{{ \App\Helpers\NumerosALetras::convertir($ventas->first()->numero_cuotas) }}</span>) CUOTAS 
                ESPECIFICADAS POR LOTE; EN CASO DE QUE EL COMPRADOR YA ESTE OCUPANDO LOS LOTES 
                RESPECTIVOS, ESTE SE VERA OBLIGADO A DESALOJARLOS EN UN PLAZO MAXIMO DE 48 HORAS,
        ...
        <div class="section-title">LEGISLACIÓN Y ARTICULOS CORRESPONDIENTE DE NUESTRO CODIGO CIVIL</div>
        <ol>
